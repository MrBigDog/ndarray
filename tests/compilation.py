...
__all__ = ("Compiler", "SnippetFormatter", "ExpectedCompileFailures")


COMPILE_COMMAND_CMAKE_TARGET = "tests/main.cpp"

OUTPUT_DIR = "expected_compile_failures"
...
class SnippetFormatter:
    ...
    prefix : `str` or `bool`
        A block of code inserted after ``preamble`` and before ``lines``.
        Defaults to the opening of the main() function.
    suffix : `str` or `bool`
        A block of code inserted after ``lines``.
        Defaults to the opening closing brace of the main() function
        started by ``prefix``.
    indent : `str` or `bool`
        Indentation to add to every line in ``lines``.
    ...
    def __call__(self, *lines):
        """Format the given lines of code."""
        b = io.StringIO()
...
class ExpectedCompileFailures:
    """A unittest.TestCase mixin for asserting compiler failures.
    """

    def assertDoesNotCompile(self, code, name=None, stderr_regex=None, stdout_regex=None,
                             output_dir=None, **kwds):
        ...
        if name is None:
            ...
            name = "{}_{}_{}".format(module, frame.lineno, frame.function)
        ...
        if stderr_regex is not None:
            self.assertIsNotNone(
                re.search(stderr_regex, process.stderr),
        ...
        if stdout_regex is not None:
            self.assertIsNotNone(
                re.search(stdout_regex, process.stdout),
